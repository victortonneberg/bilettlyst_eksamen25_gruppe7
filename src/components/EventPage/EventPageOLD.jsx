      ...
      for (const name of festivalNames) {
        try {
          const response = await fetch(
            `https://app.ticketmaster.com/discovery/v2/events.json?apikey=60AvIrywUE1YBzsifx3Ww1tx070LmuFq&keyword=${name}&locale=*&countryCode=NO`
          );
          ...
          console.error(
            `Feil ved henting av eventer for festivalen ${name}:`,
            error
          );
        }
      }
                ...
                location={
                  festivalEvent?._embedded?.venues?.[0]
                    ? `${festivalEvent._embedded.venues[0].name}, 
                            ...
                            ${festivalEvent._embedded.venues[0].city.name}`
                    : "Ukjent sted"
                }
